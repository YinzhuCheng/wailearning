...
test.describe('E2E scenarios: boundary / dynamic / complex', () => {
  ...
  test('boundary: admin creates a new student user aligned to class', async ({ page }) => {
    ...
    const createDlg = page.getByRole('dialog', { name: '新建用户' })
    await createDlg.getByLabel('用户名').fill(uname)
    await createDlg.getByLabel('密码').fill(s.password_teacher_student)
    await createDlg.getByLabel('姓名').fill('E2E新建学生')
    await createDlg.locator('label.el-radio').filter({ hasText: '学生' }).click()
    await openDialogSelectByLabel(page, '所属班级')
    await page.getByRole('option', { name: s.class_name_1 }).click()
