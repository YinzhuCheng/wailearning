  ...
  <div class="settings-container">
    ...
    <el-card class="preview-card">
      ...
      <el-alert
        ...
        :closable="false"
        class="llm-notice"
        title="学生个人日 token 上限由下方默认值或批量覆盖决定；自然日与预占/用量统计统一使用「额度统计时区」。字符/token 与单图 token 估算用于自动评分前的输入侧预占（管理员在此统一配置）。并发数为同时处理的自动评分任务数上限。"
      />
      <el-form v-loading="llmQuotaLoading" label-width="200px" style="max-width: 640px">
        ...
        <el-form-item label="字符 / token 估算">
          <el-input-number v-model="llmQuotaForm.estimated_chars_per_token" :min="0.5" :step="0.5" :precision="1" style="width: 100%" />
        </el-form-item>
        <el-form-item label="单图 token 估算">
          <el-input-number v-model="llmQuotaForm.estimated_image_tokens" :min="1" :step="100" style="width: 100%" />
        </el-form-item>
        <el-form-item>
