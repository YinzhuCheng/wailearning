...
Another targeted suite: **`e2e-course-ui-markdown-reader.spec.js`** (11 cases) — **`subject_id` enrollment counts** surfaced via **学生管理** header (dashboard UI removed), **Markdown LaTeX demo** (scoped `MarkdownEditorPanel`), **sidebar** controls, **materials** layout + **MaterialRead** + discussion card, **`/teaching-calendar`** route (calendar component parity vs removed **`Dashboard.vue`**), **flat teacher sidebar** items including **教学日历** placement after **考勤管理**. Run alone with `npx playwright test e2e-course-ui-markdown-reader.spec.js` from `apps/web/admin`.
