...
Another targeted suite: **`e2e-course-ui-markdown-reader.spec.js`** (12 cases) — dashboard **`subject_id` enrollment counts** vs class roster (seeded elective with zero enrollments), **Markdown LaTeX demo** visibility (scoped to the **作业内容** `MarkdownEditorPanel`, strict-mode pitfall), **sidebar** redundant collapse control absence + **`sidebar-edge-handle`**, **materials** stacked layout (`flex-direction: column`), **`MaterialRead`** deep-link behavior (auto-sync `selected_course` from material `subject_id`; article renders before chapter DFS completes), **discussion card on full-page material reader**, **flat teacher sidebar** (no 「日常教学」 `el-sub-menu` wrapper). Run alone with `npx playwright test e2e-course-ui-markdown-reader.spec.js` from `apps/web/admin`.
