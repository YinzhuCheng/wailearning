...
| Root-local runtime artifacts are not source layout | `__pycache__/`, `.pytest_cache/`, `.pytest_tmp/`, `.pytest_tmpbasetemp/`, `.pytest-db/`, `.coverage*`, `htmlcov/`, `test-results/`, `playwright-report/`, frontend `dist` / `.vite`, package debug logs, `node_modules/`, and `.agent-run/` are working artifacts or ignored paths. Do not normalize them into committed source structure. For routine cleanup, run `python ops/scripts/dev/clean_local_artifacts.py` to inspect targets, then `python ops/scripts/dev/clean_local_artifacts.py --apply` only when the dry-run is limited to disposable cache paths or `.agent-run` housekeeping. The script removes only high-confidence Python caches / pytest scratch artifacts and aged disposable `.agent-run/tmp/` subdirectories. It keeps `.agent-run/` top-level folders sparse by moving legacy root directories into `.agent-run/archive/`, `.agent-run/logs/`, `.agent-run/screenshots/`, or `.agent-run/tools/`, and it archives miscellaneous `.agent-run` root files under `.agent-run/archive/root-files/<timestamp>/` while preserving fixed root entrypoints such as `.agent-run/local-private-paths.md`, `.agent-run/validation-history.jsonl`, and task-scoped runner scripts. Screenshots should be created under `.agent-run/screenshots/<task-or-date>/`; loose screenshots under `.agent-run/screenshots/` are moved by the cleanup script to `.agent-run/screenshots/unfiled/<timestamp>/`. It intentionally avoids virtualenvs, `node_modules`, frontend bundles, logs, CSV ledgers, update records, handoffs, reports such as `htmlcov/`, pitfall records, validation history, Playwright traces/screenshots/videos, and uploads unless a human explicitly decides those investigation artifacts are no longer needed. |
