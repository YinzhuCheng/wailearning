...
- [作业批改、学生通知与花名册同步](docs/HOMEWORK_AND_NOTIFICATIONS_CN.md)
- [数据库迁移](#数据库迁移)
- [常见问题](#常见问题)
- [使用许可](#使用许可)
...
### 3. 学生管理
- ✅ 学生档案管理（基本信息、联系方式）
- ✅ 学号管理（**班级内唯一**；同一班级下相同学号若姓名不同，接口返回 400，避免误覆盖）
...
- ✅ 用户创建、编辑、删除
- ✅ 班主任绑定班级
- ✅ **管理员调整学生用户班级**时，同步迁移花名册中「学号 = 登录用户名」的 `Student` 记录至新班，并刷新该生的必修课选课同步，避免调班后无法选上新班课程
...
- ✅ **批改结果学生通知**：教师保存评分或自动评分**成功**后，系统向匹配到的学生账号写入一条站内通知（见「通知中心」）；失败与自动重试阶段不发通知，避免刷屏
- ✅ 教师端「学生提交」列表：保存评分后优先用接口返回数据刷新当前行，再拉全表，减少分数展示滞后
...
- ✅ 通知范围：支持全校通知和班级通知；作业批改触发的通知会带上**课程维度**（`subject_id`），便于学生按课程筛选
- ✅ 权限控制：管理员可发全校/班级通知，班主任/教师只能发本班通知
- ✅ 未读标记：自动记录已读/未读状态，支持一键全部已读
- ✅ **系统类作业通知**：标题形如「作业已批改：{作业标题}」，内容为当前展示分、满分、评语与来源（教师批改 / 自动评分等）；与上一次**完全相同**的内容不会重复插入
...
- **学生如何收到批改提醒**：学生登录名须与花名册**学号**一致且班级一致；批改完成后在「通知中心」查看；改分后若结果变化会生成新的通知内容（与上次全文相同则去重）
...
│   ├── homework_notifications.py # 作业批改完成后写入学生可见通知
│   ├── auth.py                  # 认证逻辑
│   ├── config.py                # 配置文件
│   ├── database.py              # 数据库连接
│   └── main.py                  # 应用入口
...
| 通知 | GET | /api/notifications | 获取通知列表（含未读统计；学生可见教师发布与作业批改系统通知） |
...
**问题**: 教师已评分或自动评分成功，但学生通知中心没有相关条目。

**解决**:
- 确认学生**用户账号的登录名（username）**与花名册中的**学号（student_no）**一致，且学生账号的**班级**与花名册班级一致（系统据此匹配学生用户）。
- 确认该作业关联了课程时，学生已加入该课程选课（与提交作业的前置条件一致）。
- 自动评分**失败**或排队重试阶段**不会**发此类通知；请让学生查看作业详情中的任务状态与错误说明。
